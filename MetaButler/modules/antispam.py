...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "That user is part of the MetaButler\nI can't act against our own."
        )
        return
    ...
    if int(user_id) in WHITELIST_USERS:
        message.reply_text("That's a Whitelisted! They cannot be banned!")
        return
...
@metamsg((Filters.all & Filters.chat_type.groups), can_disable=False, group=GBAN_ENFORCE_GROUP)
